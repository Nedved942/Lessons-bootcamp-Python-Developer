...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///database.db"  # Ścieżka do naszej bazy danych

db = SQLAlchemy(app)


class Users(db.Model):  # Tworzymy reprezentację tabeli w postaci modelu Pythonowego
    id = db.Column(db.Integer, primary_key=True)  # Tworzymy kolumnę o nazwie "id", jej wartością będą Integer (liczba),
    # będzie to główny klucz tabeli (primary_key=True)
    email = db.Column(db.String)  # Tworzymy kolumnę "email", która będzie tekstem
    password = db.Column(db.String)  # Tworzymy kolumnę "password", która będzie tekstem


with app.app_context():  # Budujemy bazę danych
    db.create_all()
